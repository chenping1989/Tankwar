...
class PlayerTank(pygame.sprite.Sprite):
    ...
    def shoot(self):
        ...
        if not self.is_bullet_cooling:
            ...
            return Bullet(bullet_image_paths=self.bullet_image_paths, screensize=self.screensize,
                          direction=self.direction, position=position, border_len=self.border_len,
                          is_stronger=is_stronger, speed=speed)
        return False
...
class EnemyTank(pygame.sprite.Sprite):
    ...
    def update(self, scene_elems, player_tank_group, enemy_tank_group, home):
        ...
        # 出生后实时更新
            ...
            # --坦克移动
            self.move(scene_elems, player_tank_group, enemy_tank_group, home)
            # --坦克子弹冷却更新
            if self.is_bullet_cooling:
                self.bullet_cooling_count += 1
                if self.bullet_cooling_count >= self.bullet_cooling_time:
