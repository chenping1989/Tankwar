...
class GameLevel():
    ...
    def __showGamePanel(self, screen, tank_player1, tank_player2=None):
        ...
        player2_state_tip = self.font.render('Life: %s' % tank_player2.num_lifes, True,
                                             color_white) if tank_player2 else self.font.render('Life: None', True,
                                                                                                color_white)
        player2_state_tip_rect = player2_state_tip.get_rect()
        ...
        player2_state_tip = self.font.render('TLevel: %s' % tank_player2.tanklevel, True,
                                             color_white) if tank_player2 else self.font.render('TLevel: None', True,
                                                                                                color_white)
        player2_state_tip_rect = player2_state_tip.get_rect()
    ...
    def __parserlevelfile(self):
        ...
        for line in f.readlines():
            ...
            # 大本营位置
                ...
                self.home_position = (self.border_len + self.home_position[0] * self.grid_size,
                                      self.border_len + self.home_position[1] * self.grid_size)
            # 大本营周围位置
                ...
                self.home_around_positions = [
                    (self.border_len + pos[0] * self.grid_size, self.border_len + pos[1] * self.grid_size) for pos in
                    self.home_around_positions]
            # 我方坦克初始位置
